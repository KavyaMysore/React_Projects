...
const Test = () => {
   ...
   let [state,updated]= useState(movie)
   function remove(title){
    updated(state.filter((data)=>data.Title!=title))
    alert(title+ " is deleted ")
   }
  return (
    <div className='movies'>
       <Test1 user={state} fn={remove} title={"All Movies"}/>
       <Test1 user={state.filter((data)=>(data.Year>2012))} fn={remove} title={"Movies which are released after 2012"}/>
       <Test1 user={state.filter((data)=>(data.imdbRating>8))} fn={remove} title={"Movies which have imdbRating more than 8"}/>
